...
class Camera:
    ...
    ## For camera chunked jpg
    chunked_prev_int = 0x00
...
#                 print('TODO: Save and close file?')
    ...
    def chunkedJPG(self,chunk_size=1024):
        chunk = b''
        current_bit = 0
        ...
        while(self.received_length):
            ...
            image_data_int = int.from_bytes(image_data, 1)
            
            chunk += image_data
            
            if (self.chunked_prev_int == 0xff) and (image_data_int == 0xd9):
                break
            self.chunked_prev_int = image_data_int
            if(current_bit > chunk_size):
                return (chunk, False)
            current_bit += 1
